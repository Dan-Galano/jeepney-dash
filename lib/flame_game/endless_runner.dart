...
class EndlessRunner extends FlameGame<EndlessWorld> with HasCollisionDetection {
  ...
  Future<void> onLoad() async {
    ...
    int levelDisplay = level.number;
    final scoreText = 'Violations: 0 / ${level.winScore}';
    const coinText = 'Coins: 0';
    final levelText = 'Level: $levelDisplay';
    ...
    final coinComponent = TextComponent(
      text: coinText,
      position: Vector2.all(90),
      textRenderer: textRenderer,
    );

    final levelComponent = TextComponent(
      text: levelText,
    ...
    camera.viewport.add(coinComponent);
    camera.viewport.add(levelComponent);
    ...
    world.pointsNotifier.addListener(() {
      coinComponent.text =
          coinText.replaceFirst('0', '${world.pointsNotifier.value}');
    });
  }
}
