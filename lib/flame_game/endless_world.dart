...
class EndlessWorld extends World with TapCallbacks, HasGameReference {
  ...
  final scoreNotifier = ValueNotifier(0); //violation count
  final pointsNotifier = ValueNotifier(0); //violation count
  late final Player player;
  late final DateTime timeStarted;
  Vector2 get size => (parent as FlameGame).size;
  int levelCompletedIn = 0; //time level completed
  ...
  Future<void> onLoad() async {
    ...
    scoreNotifier.addListener(() {
      ...
      if (scoreNotifier.value >= level.winScore) {
        ...
        levelCompletedIn =
            levelTime.round() * pointsNotifier.value; //multiply coins here
  ...
  void addScore({int amount = 1}) {
    ...
    pointsNotifier.value += amount; //add coins
  }
